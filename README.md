...
- `ZADOO_TURBO_PROFILE`, `ZADOO_TURBO_MAX_FPS`, `ZADOO_TURBO_CAPTURE`, and `ZADOO_TURBO_ENCODER` can force a specific profile or backend for diagnostics. Default laptop target is `540p30`; upgrades through `540p100` happen only after a local benchmark passes.
